...
class TorchscriptExporter(ModelExporter):
    ...
    def export(self, pytorch_model, input_ids, input_mask, token_type_ids, export_filename="traced_model.pt"):
        ...
        traced_script_module = torch.jit.trace(pytorch_model, [input_ids, input_mask])
...
def ids_tensor(shape, vocab_size, rng=None, name=None, type=torch.long):
    ...
    return torch.tensor(data=values, dtype=type, device='cpu').view(shape).contiguous()
...
def bert_exporter_cli(input_dir, output_dir, target_format, num_labels, debug):
    setup_logging(debug)
