...
def exporter_cli(input_dir, output_dir, debug):
    ...
    traced_script_module = torch.jit.trace(model_pytorch, [input_ids, input_mask])
    # traced_script_module = torch.jit.script(model_pytorch(**dummy_input)[1])
    traced_model_path = os.path.join(output_dir, "traced_bert_model.pt")
    logger.info("Saving torchscript model to: {}".format(traced_model_path))
